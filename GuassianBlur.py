...
def estimate_noise(I):
  ...
  return sigma
print ("##### Running GAUSSIAN filter on different noises #### ")
image = cv2.imread('images/Gauss_Ayush_Marriage.jpg')
image2 = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
before_quality = estimate_noise(image2)
print ("Estimating quality for guassian image before Gaussian filter %8.4f " % (before_quality) )
figure_size = 9
new_image = cv2.GaussianBlur(image2,(figure_size, figure_size),0)
after_quality = estimate_noise(new_image)
print ("Estimating quality for guassian image after Gaussian filter %8.4f \n " % (after_quality))
...
image2 = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
before_quality = estimate_noise(image2)
print ("Estimating quality for poisson image before Gaussian filter  %8.4f" % (before_quality))
figure_size = 9
new_image = cv2.GaussianBlur(image2,(figure_size, figure_size),0)
after_quality = estimate_noise(new_image)
print ("Estimating quality for poisson image after Gaussian filter  %8.4f \n" % (after_quality))
plt.figure(figsize=(11,6))
plt.subplot(121), plt.imshow(image2, cmap='gray'),plt.title('Original')
plt.xticks([]), plt.yticks([])
plt.subplot(122), plt.imshow(new_image, cmap='gray'),plt.title('Mean filter')
...
image2 = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
before_quality = estimate_noise(image2)
print ("Estimating quality for S&P image before Gaussian filter  %8.4f" % (before_quality))
figure_size = 9
new_image = cv2.GaussianBlur(image2,(figure_size, figure_size),0)
after_quality = estimate_noise(new_image)
...
new_image = cv2.GaussianBlur(image2,(figure_size, figure_size),0)
after_quality = estimate_noise(new_image)
print ("Estimating quality for speckle image after Gaussian filter  %8.4f \n" % (after_quality))
plt.figure(figsize=(11,6))
plt.subplot(121), plt.imshow(image2, cmap='gray'),plt.title('Original')
plt.xticks([]), plt.yticks([])
plt.subplot(122), plt.imshow(new_image, cmap='gray'),plt.title('Mean filter')
